...
def calculate_life_plan(current_age, retire_age, death_age, 
    ...
    """人生100年時代の資産推移を計算する"""
    # 税金調整（NISAでなければ利益に課税：利回りを80%にする）
    ...
    data.append({
        "年齢": current_age,
        "資産額": int(current_asset),
        "実質価値": int(current_asset),
        "元本": int(total_principal),
        "フェーズ": "現在"
    })
...
def get_side_hustle_suggestion(gap_amount):
    ...
    if gap_amount <= 0:
        ...
        **難易度**: ⭐ (誰でもすぐ可能)
        ...
        return "✏️ Level 2: Webライター・軽作業", """
        - **Webライター**: クラウドソーシングで記事作成
        - **データ入力**: 隙間時間でコツコツ作業
        **難易度**: ⭐⭐ (少しの慣れが必要)
        """, "warning"
        
    elif gap_amount <= 5.0:
        return "💻 Level 3: 動画編集・スキル販売", """
        - **動画編集**: YouTube動画の編集代行
        - **Webデザイン**: バナー作成やLPデザイン
        **難易度**: ⭐⭐⭐ (学習コストあり)
        """, "warning"
        
    else:
        return "🚀 Level 4: 転職・事業・投資強化", """
        - **転職**: 本業の年収アップを検討
        - **プログラミング**: 高単価な開発案件への挑戦
        **難易度**: ⭐⭐⭐⭐⭐ (大きな転換点)
        """, "error"